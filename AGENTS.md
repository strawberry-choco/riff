...
cargo clippy                     # lint (no custom config, all default)
cargo fmt                        # format (no custom config, all default)
```
...
No rustfmt, clippy, or CI config files exist. All defaults apply. Architecture rules live in `.lattice/standards/architecture.md`. Feature requirements live in `.lattice/requirements/features/` (generated by the requirement-forge skill).
