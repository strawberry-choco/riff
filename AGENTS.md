...
Domain (`Track`, `PlaybackQueue`, `PlaybackState`, `TrackId`) must not import anything from `app/`, `infra/`, or `ui/`. App defines traits (`AudioDecoder`, `AudioOutput`, `MetadataReader`, `CoverLoader`) that infra implements. Full architecture reference in `.lattice/standards/architecture.md` — some named modules there (e.g. `playback_engine.rs`, `app_window.rs`) differ from actual filenames; trust the actual source tree over that doc.
...
`clippy.toml` configures Clippy (msrv, tool-level options). Lint levels are set in `Cargo.toml` under `[lints.clippy]` (pedantic with selected allowances). No rustfmt or CI config files exist. Architecture rules live in `.lattice/standards/architecture.md`. Feature requirements live in `.lattice/requirements/features/` (generated by the requirement-forge skill).
