...
- **Per-crate suites**, placed with the code they cover. `riff-infra` hosts its own integration-test crate (`riff-infra/tests/mod.rs`, `autotests = false`, one `[[test]]` target named `integration`) for the real-SQLite store tests and the adapter tests. `riff-persistence` has a `tests/` suite for the generation-keyed cache primitive its read path rides on, and `riff-persistence`, `riff-library`, `riff-playback`, and `riff-backend` additionally carry inline `#[cfg(test)]` modules beside the pure logic they pin. `riff-gui` has one too — `crates/riff-gui/tests/scroll_memory_tests.rs`, the Scroll Memory's protocol asserted at its own interface — which is a **deliberate exception** to the "UI behavior is exercised from the workspace root" rule, made in the same direction the P1 note below recommends: a module whose contract is pure, deterministic and in-process is cheapest to pin where it lives, and its tests need neither kittest nor the adapter stack. The exception is only about *which* module the assertions sit in; every cross-crate, whole-frame and golden test still belongs to the root suite.
...
The workspace currently contains 762 `#[test]` functions: 587 in the workspace-root suite (55 in `domain_tests.rs`, 195 in `app_tests.rs`, 248 in `ui_tests.rs`, 73 in `golden_tests.rs`, 16 in `integration_tests.rs` — the root `infra_tests.rs` file is gone, deleted with the duplicate port family it asserted against), 128 in the `riff-infra` suite (99 in `store_tests.rs`, 29 in `adapter_tests.rs`), and 47 across the per-crate suites — `riff-playback` (28, all inline), `riff-gui` (9 in its `tests/` suite), `riff-backend` (6 inline), and `riff-persistence` and `riff-library` (2 each).

### Build status

The suites compile and run green: `cargo test --all-targets` builds every workspace crate and all four integration-test targets (the workspace root, `riff-infra`, `riff-persistence`, and `riff-gui`) and executes all 762 tests (0 failed, 0 ignored). `cargo fmt --check` and `cargo clippy --all-targets` (pedantic, `-D warnings` in CI) are part of the same quality gate, run on Linux and Windows runners by CI (`.github/workflows/ci.yml`). Two `riff-infra` tests are known and open, both for reasons in the test rather than the product: `adapter_tests::test_filesystem_watcher_forwards_debounced_audio_batches` is timing-sensitive and intermittently fails under whole-suite load while passing in isolation, and `store_tests::a_page_read_never_mixes_generations_with_a_concurrent_writer` asserts `total == rows.len()` on a window whose `LIMIT` the concurrent writer can outgrow — so it fails whenever the writer reaches more rows than the page asked for, and passes only while it has not. Both reproduced identically at the `cacbf74` baseline during the `deepen-owned-facts` work, whose verification runs therefore treat them as the known two.
...
- **Keep growing the pure crates' own suites.** `riff-persistence`, `riff-library`, `riff-playback`, and `riff-backend` carry in-crate tests and `riff-gui` has now seeded one, so most slice logic still covered from the root suite through the re-export surface has a place to move to. As a module's contract is purely internal, move (or add) its tests into the slice's own suite so they run without compiling the adapter stack — that compile isolation is one of the split's payoffs. Cross-crate integration, UI, and golden suites stay at the workspace root.
