...
**ReplayGain Normalization.** Tracks whose tags carry ReplayGain information play at consistent loudness. The scanner reads REPLAYGAIN_TRACK_GAIN and REPLAYGAIN_TRACK_PEAK alongside the rest of the metadata, and when the feature is enabled in settings the track gain is applied at the volume-scaling stage of the audio output callback, capped by the peak value so a boosting gain cannot push samples into clipping. The setting is opt-in and takes effect on the next track; untagged tracks play untouched at their native level. The scanner also reads REPLAYGAIN_ALBUM_GAIN, which the detail panel shows as a read-only row on a track or an album — it is displayed only, and playback never applies it. Analyzing untagged libraries and album-gain leveling remain deferred (see below).
...
**Metadata Extraction.** Tags are read with lofty: title, artist, album, album artist, genre, year, and track number. ReplayGain track gain, track peak and album gain tags are read too — the first two feed the normalization described under the Audio Engine, and all three appear as read-only rows in the detail panel when the file carries them. Loudness values are never editable: the tag editor offers no ReplayGain field, so what the panel shows is always what the ripper measured. The album artist field drives album grouping (with a fallback to the track artist), which keeps compilations coherent. Files with missing tags still index and play; the UI shows only the fields that exist rather than littering the view with "Unknown" placeholders.
...
**Library Search.** A single search box filters the open Library section's own columns by entity: the Artists and Albums roots list only matching artists and albums in canonical hit order (sort control hidden), a matching track surfaces its album and artist, an artist whose name matched expands into all its albums, and drilling into a hit album shows only the tracks that match — an album whose own name matched opens its full track list instead. All Tracks filters the flat list by the query as always; the Folders tree stays browsable under a query, pruning branches that contain no matches rather than being replaced by the flat list. Matching is literal and case-insensitive (non-Latin text included), and clearing the query restores the full section columns.
