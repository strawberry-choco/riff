...
| Smart Playlists | Locally generated discovery lists: Recently Added, Most Played, Never Played, Lost Gems | implemented | P1 | Library Cache Persistence |
...
| Cache Hardening | Versioned cache format with safe fallback and a Clear Library Cache action | implemented | P2 | Library Cache Persistence |
...
**Library Cache Persistence.** The full scanned library — tracks, artists, and albums — is serialized to `library_cache.json` in the platform data-local directory (via the directories crate). The cache loads on the first frame of the first launch, so a 50,000-track collection is browsable almost instantly instead of waiting for a disk walk. It is rewritten after every completed scan and whenever a library path is removed. A missing, corrupt, or version-mismatched cache falls back to an empty library with a warning and a user-visible notice (see Cache Hardening below), and a failed cache write logs a warning without disturbing the UI.
...
**Smart Playlists.** Four discovery playlists are generated locally from data riff already has: Recently Added, Most Played, Never Played, and Lost Gems (tracks unheard for more than ninety days). Nothing leaves the machine — the lists are computed from the library index, and the play counts and timestamps they rely on persist in the library cache across restarts. They behave like any other playable list: selecting one fills the queue.

**Custom Playlists.** You can create named playlists, add tracks to them from the library's context menus (with deduplication, so a track appears once per playlist), reorder their contents, rename them, and delete them. Playlists persist in `playlists.json` in the platform data-local directory — deliberately separate from the rebuildable library cache, because playlists are user data. Entries whose files have vanished stay visible, rendered struck-through as "(missing)", and are excluded from playback rather than breaking the list.

**Cache Hardening.** The library cache carries a schema version. On load, a version mismatch or a corrupt file falls back to an empty library with a warning in the log and a user-visible notice explaining what happened, instead of a silent reset. Settings provides a "Clear Library Cache" action, guarded by a confirmation dialog, for the cases where you want to force a clean rebuild; the cache repopulates on the next scan.
