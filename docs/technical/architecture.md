...
**The read seam has one named shape.** A browse surface's total and its visible window are a **Listing Page** (`Page<T>`): one fact, read from the Application Store under one connection acquisition and stamped with the generation captured inside it, with the stamp kept private so no caller can observe an epoch at all. The nine paged listings each declare their own page read on `LibraryQueryStore` (`tracks_page`, `search_page`, `hit_albums_page`, `hit_artists_page`, `artists_page`, `albums_page`, `genres_page`, `artists_in_genre_page`, `artist_albums_in_genre_page`) rather than pairing a window read with a count read, and each states which of the store's two generations stamps it. Everything a cached level needs to decide — observe the generation once, serve while it is current, otherwise load and commit — is `GenerationCache::level` in `levels.rs`, beside the `GenerationCache` primitive it drives. Placement follows the cache primitive's own rule: the two capability crates are siblings with no edge between them, and playback's generation caches adopt the same procedure later.
